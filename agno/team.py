...
class Team:
    ...
    def run(self,
        ...
        *,
        ...
        videos: Optional[Sequence[Video]] = None,
        files: Optional[Sequence[File]] = None,
        **kwargs: Any) -> Iterator[TeamRunResponse]: ...
    ...
    def run(self,
        ...
        *,
        ...
        videos: Optional[Sequence[Video]] = None,
        files: Optional[Sequence[File]] = None,
        **kwargs: Any) -> Union[TeamRunResponse, Iterator[TeamRunResponse]]:
        ...
        for attempt in range(num_attempts):
            ...
            if self.mode == "route":
                ...
                forward_task_func: Function = self.get_forward_task_function(message=user_message,
                    stream=stream,
                    async_mode=False,
                    images=images,
                    videos=videos,
                ...
                _tools.append(self.get_transfer_task_function(stream=stream,
                        async_mode=False,
                        images=images,
                        videos=videos,
                        audio=audio,
                ...
                run_member_agents_func = self.get_run_member_agents_function(stream=stream,
                    async_mode=False,
                    images=images,
                    videos=videos,
                    audio=audio,
            ...
            try:
                ...
                if self.mode == "route":
                    run_messages: RunMessages = self.get_run_messages(run_response=self.run_response,
                        message=message,
                        audio=audio,
                        images=images,
                        videos=videos,
                    ...
                    run_messages = self.get_run_messages(run_response=self.run_response,
                        message=message,
                        audio=audio,
                        images=images,
                        videos=videos,
                        ...
                        **kwargs)
                if stream:
                    resp = self._run_stream(run_response=self.run_response,
                        run_messages=run_messages,
                        stream_intermediate_steps=stream_intermediate_steps)
                    return resp
                else:
                    self._run(run_response=self.run_response,
                        run_messages=run_messages)
                ...
                return TeamRunResponse(run_id=self.run_id or str(uuid4()),
    ...
    def _run_stream(self,
        ...
        self._log_team_run()
        if stream_intermediate_steps:
            yield self._create_run_response(from_run_response=run_response,
    ...
    async def arun(self,
        ...
        *,
        ...
        videos: Optional[Sequence[Video]] = None,
        files: Optional[Sequence[File]] = None,
        **kwargs: Any) -> AsyncIterator[TeamRunResponse]: ...
    async def arun(self,
        message: Union[str, List, Dict, Message],
        *,
        stream: bool = False,
        ...
        videos: Optional[Sequence[Video]] = None,
        files: Optional[Sequence[File]] = None,
        **kwargs: Any) -> Union[TeamRunResponse, AsyncIterator[TeamRunResponse]]:
            ...
            try:
                ...
                if self.mode == "route":
                    ...
                    run_messages = self.get_run_messages(run_response=self.run_response,
                        ...
                        **kwargs)
                if stream:
                    resp = self._arun_stream(run_response=self.run_response,
                        run_messages=run_messages,
                        stream_intermediate_steps=stream_intermediate_steps)
                    return resp
                else:
                    await self._arun(run_response=self.run_response,
                        run_messages=run_messages)
                    return self.run_response
            except ModelProviderError as e:
                print(f"Attempt {attempt + 1}/{num_attempts} failed: {str(e)}")
    ...
    def _print_response(self,
        ...
        **kwargs: Any) -> None:
        ...
        with Live(console=console) as live_console:
            ...
            # Run the agent
            run_response: TeamRunResponse = self.run(message=message,
                images=images,
                audio=audio,
                videos=videos,
                files=files,
            ...
            if isinstance(run_response, TeamRunResponse):
                ...
                if self.show_members_responses:
                    for member_response in run_response.member_responses:
                        ...
                        member_response_content: Union[str, JSON, Markdown] = self._parse_response_content(member_response,
                ...
                if self.show_tool_calls and run_response.tools:
                    ...
                    if formatted_calls:
                        ...
                        lines = []
                        for call in formatted_calls:
                            wrapped_call = textwrap.fill(f"• {call}", width=panel_width, subsequent_indent="  ")  # Indent continuation lines
                            lines.append(wrapped_call)
                        # Join with blank lines between items
                        tool_calls_text = "\n\n".join(lines)
                        team_tool_calls_panel = create_panel(content=tool_calls_text,
    ...
    def _print_response_stream(self,
        ...
        **kwargs: Any) -> None:
        ...
        with Live(console=console) as live_console:
            ...
            stream_resp = self.run(message=message,
                audio=audio,
                images=images,
                videos=videos,
                files=files,
            ...
            for resp in stream_resp:
                ...
                for member_response in resp.member_responses if hasattr(resp, "member_responses") else []:
                    ...
                    if self.show_members_responses and member_id is not None:
                        ...
                        if markdown:
                            show_markdown = True
                        member_response_content = self._parse_response_content(member_response,
                            tags_to_include_in_markdown,
                            show_markdown=show_markdown)
                        member_response_panel = create_panel(content=member_response_content,
    ...
    async def aprint_response(self,
        ...
        **kwargs: Any) -> None:
        ...
        if self.response_model is not None:
            stream = False
        if stream:
            await self._aprint_response_stream(message=message,
                console=console,
                show_message=show_message,
                show_reasoning=show_reasoning,
                show_reasoning_verbose=show_reasoning_verbose,
            ...
            await self._aprint_response(message=message,
                console=console,
                show_message=show_message,
                show_reasoning=show_reasoning,
                show_reasoning_verbose=show_reasoning_verbose,
                ...
                markdown=markdown,
                **kwargs)
    async def _aprint_response(self,
        message: Optional[Union[List, Dict, str, Message]] = None,
        console: Optional[Any] = None,
        show_message: bool = True,
        show_reasoning: bool = True,
        ...
        **kwargs: Any) -> None:
        ...
        with Live(console=console) as live_console:
            ...
            if message and show_message:
                ...
                live_console.update(Group(*panels))
            # Run the agent
            run_response: TeamRunResponse = await self.arun(message=message,
                images=images,
                audio=audio,
                videos=videos,
                files=files,
            ...
            panels = [p for p in panels if not isinstance(p, Status)]
            live_console.update(Group(*panels))
    async def _aprint_response_stream(self,
        message: Optional[Union[List, Dict, str, Message]] = None,
        console: Optional[Any] = None,
        show_message: bool = True,
        show_reasoning: bool = True,
        ...
        **kwargs: Any) -> None:
        ...
        with Live(console=console) as live_console:
            ...
            if render:
                live_console.update(Group(*panels))
            # Get response from the team
            stream_resp = await self.arun(message=message,
                audio=audio,
                images=images,
                videos=videos,
                files=files,
    ...
    def _reason(self, run_response: TeamRunResponse, run_messages: RunMessages, stream_intermediate_steps: bool = False) -> Iterator[TeamRunResponse]:
        ...
        if reasoning_model_provided and self.reasoning_model is not None:
            ...
            if (self.reasoning_model.__class__.__name__ == "DeepSeek"
            ...
            reasoning_agent: Agent = get_default_reasoning_agent(reasoning_model=reasoning_model,
                ...
                try:
                    ...
                    if stream_intermediate_steps:
                        for reasoning_step in reasoning_steps:
                            yield self._create_run_response(content=reasoning_step,
        ...
        if stream_intermediate_steps:
            yield self._create_run_response(content=ReasoningSteps(reasoning_steps=[ReasoningStep(result=reasoning_message.content)]),
    ...
    def _create_run_response(self,
        ...
        rr = TeamRunResponse(run_id=self.run_id,
            session_id=self.session_id,
            team_id=self.team_id,
            content=content,
            thinking=thinking,
    ...
    def _configure_model(self, show_tool_calls: bool = False) -> None:
        ...
        if self.model is None:
            try:
                ...
                print(e)
                print("Agno agents use `openai` as the default model provider. "
    ...
    def get_system_message(self,
        ...
        if self.mode == "coordinate":
            ...
            system_message_content += "You are the leader of a team of AI Agents and possible Sub-Teams:\n"
            system_message_content += self.get_members_system_message_content()
            system_message_content += ("- You act as a router for the user's request. You have to choose the correct agent(s) to forward the user's request to. This should be the agent that has the highest likelihood of completing the task.\n"
            ...
            system_message_content += self.get_members_system_message_content()
            system_message_content += ("- Only call run_member_agent once for all agents in the team.\n"
    ...
    def get_run_member_agents_function(self,
        ...
        def run_member_agents(task_description: str, expected_output: Optional[str] = None) -> Iterator[str]:
            ...
            for member_agent_index, member_agent in enumerate(self.members):
                ...
                self.memory = cast(TeamMemory, self.memory)
                self.memory.add_interaction_to_team_context(member_name=member_name,
    ...
    def _get_team_session(self) -> TeamSession:
        ...
        return TeamSession(session_id=self.session_id,
            team_id=self.team_id,
