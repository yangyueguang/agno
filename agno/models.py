...
class MessageMetrics:
    ...
    def __add__(self, other: "MessageMetrics") -> "MessageMetrics":
        result = MessageMetrics(input_tokens=self.input_tokens + other.input_tokens,
...
class Model(ABC):
    ...
    def _create_function_call_result(self, fc: FunctionCall, success: bool, output: Optional[Union[List[Any], str]], timer: Timer) -> Message:
        return Message(role=self.tool_message_role, content=output if success else fc.error, tool_call_id=fc.call_id, tool_name=fc.function.name,
                       tool_args=fc.arguments, tool_call_error=not success, stop_after_tool_call=fc.function.stop_after_tool_call, metrics=MessageMetrics(time=timer.elapsed))
    ...
    def run_function_calls(self, function_calls: List[FunctionCall], function_call_results: List[Message]) -> Iterator[ModelResponse]:
        ...
        for fc in function_calls:
            ...
            yield ModelResponse(content=fc.get_call_str(),
                                tool_calls=[{"role": self.tool_message_role, "tool_call_id": fc.call_id, "tool_name": fc.function.name, "tool_args": fc.arguments}],
            ...
            yield ModelResponse(content=f"{fc.get_call_str()} completed in {function_call_timer.elapsed:.4f}s.",
