...
class SqliteStorage(Storage):
    def __init__(self, table_name: str, db_url: Optional[str] = None, db_file: Optional[str] = None, db_engine: Optional[Engine] = None, schema_version: int = 1, auto_upgrade_schema: bool = False, mode: Optional[Literal['agent', 'team', 'workflow']] = 'agent'):
        super().__init__(mode)
        _engine: Optional[Engine] = db_engine
        if _engine is None and db_url is not None:
    ...
    def get_table_v1(self) -> Table:
        common_columns = [Column('session_id', String, primary_key=True), Column('user_id', String, index=True), Column('memory', sqlite.JSON), Column('session_data', sqlite.JSON), Column('extra_data', sqlite.JSON), Column('created_at', sqlite.INTEGER, default=lambda: int(time.time())), Column('updated_at', sqlite.INTEGER, onupdate=lambda: int(time.time())), ]
        specific_columns = []
        if self.mode == 'agent':
    ...
    def table_exists(self) -> bool:
        try:
            with self.SqlSession() as sess:
                result = sess.execute(text('SELECT name FROM sqlite_master WHERE type="table" AND name=:table_name'), {'table_name': self.table_name}).scalar()
    ...
    def create(self) -> None:
        ...
        if not self.table_exists():
            ...
            try:
                table_without_indexes = Table(self.table_name, MetaData(), *[c.copy() for c in self.table.columns])
    ...
    def upsert(self, session: Session, create_and_retry: bool = True) -> Optional[Session]:
        ...
        try:
            with self.SqlSession() as sess, sess.begin():
                if self.mode == 'agent':
                    stmt = sqlite.insert(self.table).values(session_id=session.session_id, agent_id=session.agent_id, team_session_id=session.team_session_id, user_id=session.user_id, memory=session.memory, agent_data=session.agent_data, session_data=session.session_data, extra_data=session.extra_data)
                    stmt = stmt.on_conflict_do_update(index_elements=['session_id'], set_=dict(agent_id=session.agent_id, team_session_id=session.team_session_id, user_id=session.user_id, memory=session.memory, agent_data=session.agent_data, session_data=session.session_data, extra_data=session.extra_data, updated_at=int(time.time())))
                elif self.mode == 'team':
                    stmt = sqlite.insert(self.table).values(session_id=session.session_id, team_id=session.team_id, user_id=session.user_id, team_session_id=session.team_session_id, memory=session.memory, team_data=session.team_data, session_data=session.session_data, extra_data=session.extra_data)
                    stmt = stmt.on_conflict_do_update(index_elements=['session_id'], set_=dict(team_id=session.team_id, user_id=session.user_id, team_session_id=session.team_session_id, memory=session.memory, team_data=session.team_data, session_data=session.session_data, extra_data=session.extra_data, updated_at=int(time.time())))
                elif self.mode == 'workflow':
                    stmt = sqlite.insert(self.table).values(session_id=session.session_id, workflow_id=session.workflow_id, user_id=session.user_id, memory=session.memory, workflow_data=session.workflow_data, session_data=session.session_data, extra_data=session.extra_data)
                    stmt = stmt.on_conflict_do_update(index_elements=['session_id'], set_=dict(workflow_id=session.workflow_id, user_id=session.user_id, memory=session.memory, workflow_data=session.workflow_data, session_data=session.session_data, extra_data=session.extra_data, updated_at=int(time.time())))
                sess.execute(stmt)
        except Exception as e:
